...
LORA=/dataset1/zailong/models/spectral-surgery/Qwen3-8B-InstructionFollowing-LoRA
HNS=/dataset1/zailong/models/spectral-surgery/Qwen3-8B-InstructionFollowing-SpectralSurgery-HNS4p1
DATA=/dataset1/zailong/data/peft-sft-lab/cross-task-mechanism
TRAIN="$DATA/tulu-3-sft-personas-instruction-following-train.parquet"
RUN_ROOT=/dataset1/zailong/runs/peft-sft-lab/Qwen3-8B/hns-cross-task-20260908/tulu
...
if [[ ! -f "$RUN_ROOT/activation/activation_analysis.json" ]]; then
  "$PYTHON" scripts/measure_lora_modification.py \
    ...
    --dataset_path "$TRAIN" --dataset_kind tulu --samples 256 \
...
if [[ ! -f "$RUN_ROOT/eval/lora/ifeval/metrics.json" ]]; then
  cp -a "$LORA/eval-ifeval" "$RUN_ROOT/eval/lora/ifeval"
fi
if [[ ! -f "$RUN_ROOT/eval/hns/ifeval/metrics.json" ]]; then
  cp -a "$HNS/eval-ifeval" "$RUN_ROOT/eval/hns/ifeval"
fi

for label in scalar_shrink shape_only head_only tail_only; do
  adapter_label=${label//_/-}
  output="$RUN_ROOT/eval/$label/ifeval"
  if [[ ! -f "$output/metrics.json" ]]; then
    "$PYTHON" -m finetune.eval.eval_ifeval \
      --base_model "$BASE" --adapter_dir "$ADAPTERS/$adapter_label" --output_dir "$output" \
      --dataset_path "$DATA/ifeval-train.jsonl" --split train \
  ...
  "$PYTHON" scripts/summarize_cross_task_task.py "$RUN_ROOT" --task tulu
done

"$PYTHON" scripts/summarize_cross_task_task.py "$RUN_ROOT" --task tulu
echo "[Done] Tulu mechanism experiment: $RUN_ROOT"
