...
RUN_ROOT=/dataset1/zailong/runs/peft-sft-lab/hns-restored-standardization-2x4-20260911
...
"$PYTHON" scripts/build_2x4_restored_standardization.py \
...
"$PYTHON" scripts/eval_forgetting_matrix_vllm.py \
  ...
  --adapter_block_size 6 --prompt_chunk_short 1024 --prompt_chunk_long 64 --seed 42

"$PYTHON" scripts/score_forgetting_matrix.py --matrix_dir "$EVAL_ROOT" --workers 16
echo "[Done] $BASE_NAME restored standardization"
